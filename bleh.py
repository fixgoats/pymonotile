...
a = 1
N = 20
grid = np.array([[a * i, a * j] for i in range(N) for j in range(N)])
...
density = np.zeros((n_es, nk))
for j, e in enumerate(es):
    for i, k in enumerate(krange):
        delta = eigvecs @ np.diag(np.exp(-fuzz * (eigvals - e) ** 2)) @ inv_eigvecs
        k_vec = (1 / N) * np.array([np.exp(1j * k * r[0]) for r in grid])
...
im = ax.imshow(density, interpolation="none")
